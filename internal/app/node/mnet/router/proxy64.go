...
import (
	...
	"n2x.dev/x-node/internal/app/node/mnet/proxy"
...
func getProxy64ConnectionID(ipv4 string, proto layers.IPProtocol, port uint16) proxy64ConnID {
	return proxy64ConnID(fmt.Sprintf("n2x64:%s:%s:%d", ipv4, proto.String(), port))
}
...
func (r *router) proxy64GC() {
	...
	for {
		select {
		case <-ticker.C:
			xlog.Debug("Running n2x64 garbage collector...")
		...
		case <-r.proxy64.closeCh:
			xlog.Debug("Closing n2x64 garbage collector")
			return
		}
	}
}
