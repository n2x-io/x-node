...
const n2xIPv6Prefix string = "fd77:f:"
...
func GetIPv6(ipv4 string) (string, error) {
	...
	ipv6 := fmt.Sprintf("%s%02x%02x:%02x%02x:1:0:0:0", n2xIPv6Prefix, a, b, c, d)

	return net.ParseIP(ipv6).String(), nil
}

func GetIPv6Endpoint(ipv6 string) (string, error) {
	if !strings.HasPrefix(ipv6, n2xIPv6Prefix) {
		return "", errors.Errorf("invalid n2x64 ipv6: %s", ipv6)
	}
	...
	addr := fmt.Sprintf("%s%s:%s:1:0:0:0", n2xIPv6Prefix, s[2], s[3])

	return net.ParseIP(addr).String(), nil
}

func GetN2X64Addr(ipv6, ipv4 string) (string, error) {
	if !strings.HasPrefix(ipv6, n2xIPv6Prefix) {
		return "", errors.Errorf("invalid n2x64 ipv6: %s", ipv6)
	}
	...
	addr := fmt.Sprintf("%s%s:%s:%s:%s:%s:%s", n2xIPv6Prefix, s1[2], s1[3], s2[0], s2[1], s2[2], s2[3])
...
func IsIAPIPv6Addr(ipv6 string) bool {
	prefix1 := fmt.Sprintf("%s:", n2xIPv6Prefix)
	prefix2 := fmt.Sprintf("%s0:0:0:0:", n2xIPv6Prefix)
...
func GetIPv6ByMac(ipv6 string) (net.IP, error) {
	if !strings.HasPrefix(ipv6, n2xIPv6Prefix) {
		return nil, errors.Errorf("invalid n2x64 ipv6: %s", ipv6)
	}
	...
	prefix := fmt.Sprintf("%s%s:%s::", n2xIPv6Prefix, s[2], s[3])

	n2xPrefix := net.ParseIP(prefix)
	...
	addrv6, err := eui64.ParseMAC(n2xPrefix, mac)
	if err != nil {
